...
const Product = ({ id, name, price }) => (
    <ul>
        ...
        <li className="btn"><Button message="ADD TO CART" onClick={addToCart.bind(null, id)} /></li>
    </ul>
);
//displaying cart
const CartProduct = ({ id, name, price }) => (
    <div>
        {productCount[id] > 0 ? <ul>
            ...
            <li className="productName">{name}</li>
            <li className="productPrice">{price}</li>
            <li className="productQunatity">{productCount[id]}</li></ul> : ''}
    </div>
);
...
const CartList = () => (
    <div>
        ...
        <ul className="cartList">{
            products.map(({ id, name, price }) => <CartProduct id={id} name={name} price={price} />)}</ul>
        <span className="totalDisplay"><b>Total Amount : {Totalprice}</b></span>
